...
- **`discover_catalog_candidates.py` is the one narrow exception, and it proposes rather than writes.** It takes a compatibility value only where the product's *published name* states exactly one — a kit called `DDR5-6000` is DDR5, a supply called `750 Watt` is 750W. That is reading a label, not interpreting prose, and a name stating two values (`supports ATX / Micro-ATX / Mini-ITX`) is rejected. It will not derive processor socket or GPU TDP, because neither appears in a name. Output is a proposal for review; nothing reaches MongoDB.
- **Never derive a part number from a URL slug.** It was tried and removed. Every rule that recovered `ST2000DM008` from `seagate-barracuda-2tb-hard-drive-st2000dm008` produced `BARRACUDA-2TB` or `5600X-100-100000065BOX` from the next slug. The MPN is what prices and dedup match on, so a plausible wrong one silently binds a price to the wrong product. `candidate_identity()` reads only the page's stated `mpn`, `gtin`, or a part-number-shaped `sku`, and otherwise leaves it blank for a human.
- **Sample a sitemap with a stride, not from the front.** Sitemaps are grouped by brand and product line; taking storage sequentially returned thirty NVMe drives and no SATA at all.
- **`PLAUSIBLE_PRICE_INR` flags bundles.** A "Mini ITX Case with SMPS, CPU Cooler" listed at ₹360,000 is not a case. Outliers are flagged rather than dropped, since genuinely expensive parts exist — 96GB DDR5 kits really do pass ₹1 lakh.
...
- **Ambiguity is resolved by a recorded human choice, never by code.** `--inspect-ambiguous` fetches each candidate so the report carries the retailer's title/price beside the verified catalog name; `--resolutions` then reads the operator's chosen URLs. `load_resolutions()` maps MPN → *set* of URLs because a part number can be ambiguous at several retailers, and a source resolves only when exactly one of *its own* candidates is in that set — two is contradictory, zero means the choice belongs to another retailer. A resolved page still passes through `conflicting_identifier()`, so a wrong choice is refused rather than trusted (`OperatorResolutionTests` pins that). Do not add code that picks a candidate by scoring titles.
